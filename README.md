...
```
文件说明：species_and_gene.tsv为每个样本的物种丰度、基因定量信息和分箱的定量信息等；classified_index.tsv为species_and_gene.tsv对应的分类索引文件，不输入该文件则将species_and_gene.tsv视为一个矩阵表进行分析；environmental_factor.tsv为每个样本的环境影子信息。
绘图结果：
